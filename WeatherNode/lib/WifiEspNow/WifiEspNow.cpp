...
bool
#if defined(ESP8266)
WifiEspNowClass::begin(esp_now_role role)
#elif defined(ESP32)
...
bool
#if defined(ESP8266)
WifiEspNowClass::addPeer(const uint8_t mac[6], int channel, esp_now_role role, const uint8_t key[WIFIESPNOW_KEYLEN])
#elif defined(ESP32)
WifiEspNowClass::addPeer(const uint8_t mac[6], int channel, wifi_interface_t ifidx, const uint8_t key[WIFIESPNOW_KEYLEN])
#endif
{
#if defined(ESP8266)
  if (this->hasPeer(mac)) {
  ...
  return esp_now_add_peer(const_cast<u8*>(mac), role, static_cast<u8>(channel),
